function Wolf(game) {
    ...
    this.walkForward = new Animation(ASSET_MANAGER.getAsset("./img/wolf32x28.png"), 0, 0, 32, 28, 0.3, 6, true, false);
    this.walkBackward = new Animation(ASSET_MANAGER.getAsset("./img/wolf32x28.png"), 0, 28, 32, 28, 0.3, 6, true, false);
    
    this.deathTime = 3000;
    this.lifeTime = 0;
    this.reproductiveTime = 500;
    this.birthRate = 1;
...
Wolf.prototype.collideLeft = function () {
    return (this.x - this.radius) < 0;
};

Wolf.prototype.collideRight = function () {
    return (this.x + this.radius) > 770;
};

Wolf.prototype.collideTop = function () {
    return (this.y - this.radius) < 0;
};

Wolf.prototype.collideBottom = function () {
...
Wolf.prototype.update = function () {
    ...
    if (this.lifeTime >= this.deathTime || this.energy <= 0) {
        this.removeFromWorld = true;
    }

    for (var i = 0; i < bunnies.length; i++) {
        if (collided(this, bunnies[i])) {
            if (this.energy < 800) {
                this.energy += 50;
            }
        }
